...
enum BsDatatablesClearData { always, never }
...
class BsDatatable extends StatefulWidget {
  ...
  final BsDatatablesClearData clearData;
}
...
class _BsDatatableState extends State<BsDatatable> {
  ...
  void init() {
    widget.source.controller.onReloadListener((clearData) {
      if (widget.serverSide != null) {
        _updateState(() => _processing = true);

        widget.serverSide!(widget.source.controller.toJson()).then((value) {
          widget.source.reload(clear: clearData, data: widget.source.response.data);
          _updateState(() {
            _processing = false;
        ...
        widget.source.reload(clear: false);
        _updateState(() {});
      }
    });
  ...
  String _replaceText(String text) {
    String start = text.replaceAll(
        '__START__',
        ((widget.source.controller.searchValue == '' && widget.source.countData > 0) ||
                    (widget.source.controller.searchValue != '' && widget.source.countFiltered > 0)
                ? widget.source.controller.start + 1
                : 0)
            .toString());
    String end = start.replaceAll('__END__', (widget.source.controller.start + widget.source.countDataPage).toString());
      ...
      _columns.add(TextButton(
        ...
        onPressed: column.orderable == false
            ...
            : () {
                ...
                if (column.orderState.orderType == BsOrderColumn.def || column.orderState.orderType == BsOrderColumn.desc)
                  dir = BsOrderColumn.asc;
                else if (column.orderState.orderType == BsOrderColumn.asc) dir = BsOrderColumn.desc;

                widget.source.controller.orders = [];
                _currentColumns.forEach((tempColumn) => tempColumn.orderState = BsOrderColumn(ordered: false));

                column.orderState = BsOrderColumn(ordered: true, orderType: dir);

                widget.source.controller.orders.add({'column': _currentColumns.indexOf(column).toString(), 'dir': dir});
                widget.source.controller.reload(clearData: widget.clearData == BsDatatablesClearData.always);
              },
        ...
        children.add(Material(
          ...
          child: InkWell(
            ...
            onTap: widget.onRowPressed == null
                ? null
                : () {
                    if (widget.onRowPressed != null) widget.onRowPressed!(i, cells);
                  },
            onHover: (value) {
              _updateState(() {
                _isHover[i] = value;
  ...
  Widget informationData() {
    ...
    if (widget.source.controller.searchValue != '') text = _replaceText('${widget.language.information} (${widget.language.informationFiltered})');

    return widget.textInfo != null ? widget.textInfo! : Text(text, style: TextStyle(fontSize: 12.0).merge(widget.textInfoStyle));
  }

  Widget title() {
    return DefaultTextStyle(
        style: TextStyle(fontSize: 18.0, color: Colors.black).merge(widget.titleTextStyle),
      ...
      children: [
        widget.language.perPageLabel == null
            ...
            : Container(
                ...
                child: Text(widget.language.perPageLabel!, style: TextStyle(fontSize: 10.0)),
              ),
        Container(
          width: 60.0,
          margin: EdgeInsets.only(right: 5.0),
          decoration: BoxDecoration(
              color: Colors.white, border: Border.all(color: widget.style.borderColor), borderRadius: BorderRadius.all(Radius.circular(5.0))),
          child: TextField(
            controller: _inputLength,
            decoration: InputDecoration(
              border: InputBorder.none,
              contentPadding: EdgeInsets.fromLTRB(10.0, 12.0, 10.0, 12.0),
              hintStyle: TextStyle(fontSize: 14.0, fontWeight: FontWeight.w100, color: Colors.grey),
              isDense: true,
            ),
      ...
      child: Column(
        ...
        children: [
          widget.language.searchLabel == null
              ...
              : Container(
                  margin: EdgeInsets.only(bottom: 2.0),
                  child: Text(widget.language.searchLabel!, style: TextStyle(fontSize: 10.0)),
                ),
          Container(
            decoration: BoxDecoration(
                color: Colors.white, border: Border.all(color: widget.style.borderColor), borderRadius: BorderRadius.all(Radius.circular(5.0))),
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                hintStyle: TextStyle(fontSize: 14.0, fontWeight: FontWeight.w100, color: Colors.grey).merge(widget.hintStyleSearch),
                isDense: true,
              ),
              onChanged: (value) => _doneTyping(value, (value) {
  ...
  Widget pagination() {
    ...
    if (widget.source.controller.searchValue != '') countPage = (widget.source.countFiltered / widget.source.controller.length).ceil();
        ...
        _pagination.add(Container(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              ...
              hintStyle: TextStyle(fontSize: 12.0, fontWeight: FontWeight.w100, color: Colors.grey),
              isDense: true,
            ),
            keyboardType: TextInputType.number,
            inputFormatters: [FilteringTextInputFormatter.digitsOnly],
